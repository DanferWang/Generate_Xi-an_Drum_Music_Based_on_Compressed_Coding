...
from keras.layers import Dense, LSTM, BatchNormalization, Flatten, Input, Lambda, Reshape
...
inputDir = '../'
fileName = inputDir + 'compressed_dataset.txt'  # datasetName
### output initalize
rePut1 = []
rePut2 = []
rePut3 = []
    ...
    with open(fileName, 'r') as file_to_read:
        while True:
            ...
            if not lines:
                break
                pass
            dat1t, dat2t = [float(i) for i in lines.split()]
            rePut1.append(dat1t)
            rePut2.append(dat2t)
            pass
        pass
...
dataset = np.hstack((rePutN1, rePutN2))
...
# embedding = Embedding(input_dim=pitch_size, output_dim=160, input_shape=(n_steps,n_features))(encoder_input)
...
encoder.summary()
plot_model(encoder, to_file="VAE_encoder_plot_for_compressed.png", show_shapes=True, show_layer_names=True)
...
decoder_output = Dense(2, activation="relu", name="decoder_output")(lstmT_2)

### decoder model
decoder = Model(decoder_input, decoder_output, name="decoder")
decoder.summary()
plot_model(decoder, to_file="VAE_decoder_plot_for_compressed.png", show_shapes=True, show_layer_names=True)

## VAE
vae_output = decoder(encoder(encoder_input)[2])
vae = Model(encoder_input, vae_output, name="VAE")
vae.summary()
plot_model(decoder, to_file="VAE_plot_for_compressed.png", show_shapes=True, show_layer_names=True)
...
vae_load = load_model('compressed.h5', custom_objects={'kl_divergence_loss':kl_divergence_loss})
# test
test_input = np.array(X_test).reshape((-1,1,3,3))
test_output = np.array(y_test)
test_pred = []
for i in range(len(test_input)):
    i_pred = vae_load.predict(test_input[i])[0][2]
    test_pred.append(i_pred)
